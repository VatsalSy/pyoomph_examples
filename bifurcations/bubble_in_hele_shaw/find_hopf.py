...
with DropletInCellProblem() as problem:
    ...
    # Increase the flow rate, go close to the Hopf
    # For the other Hopf (slightly asymmetric branch), use the bifurcation_gui.py to find all branches
    # Mark a point close to the bifurcation with 0-9, press "o" to write the output and load the corresponding state by problem.load_state
    problem.go_to_param(Q=0.036)
    
    # Get a guess for the eigenvector
    problem.solve_eigenproblem(10)
    problem.activate_bifurcation_tracking("Q", "hopf")
    problem.solve(max_newton_iterations=50)
    print("Found Hopf at Q=", problem.Q.value)
    
    bifurcation_curve=NumericalTextOutputFile(problem.get_output_directory("bifurcation_curve.txt"),header=["h","Q","omega"])
    bifurcation_curve.add_row(problem.h, problem.Q,numpy.imag(problem.get_last_eigenvalues()[0]))
    ...
    while problem.h.value<0.06:
        ...
        bifurcation_curve.add_row(problem.Q, problem.h,numpy.imag(problem.get_last_eigenvalues()[0]))
